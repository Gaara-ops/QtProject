...
18.镜面反射光--等下次看的时候再添加说明

19.点光源
三个基本的光照模型（环境光，漫射光和镜面反射光），这三种模型都是基于平行光的。
平行光只是通过一个向量来表示，没有光源起点，因此它不会随着距离的增大而衰减。
点光源有光源起点而且有衰减效果，距离光源越远光线越弱
真实光线的衰减是按照平方反比定律的，也就是说光线的强度和离光源的距离的平方成反比。
